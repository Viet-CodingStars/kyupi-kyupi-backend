...
func TestSignUp(t *testing.T) {
  ...
  t.Run("successful signup", func(t *testing.T) {
    ...
    body := bytes.NewBufferString(`{"email":"test@example.com","password":"password123","name":"Test User", "gender": "male", "birth_date": "2000-01-01", "target_gender": "female"}`)
    ...
    if response.User.TargetGender != "female" {
      t.Fatalf("expected target gender female, got %s", response.User.TargetGender)
    }
  })
